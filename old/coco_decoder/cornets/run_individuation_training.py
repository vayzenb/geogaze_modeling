...
parser.add_argument('--train_images', type=str, required=True,
                    help='path to training images folder (files named <ImageID>.jpg)')
parser.add_argument('--train_csv', type=str, required=True,
                    help='path to training bbox CSV')
parser.add_argument('--val_images', type=str, required=True,
                    help='path to validation images folder (files named <ImageID>.jpg)')
parser.add_argument('--val_csv', type=str, required=True,
                    help='path to validation bbox CSV')
parser.add_argument('--num_count_classes', type=int, default=6,
                    help='number of count classes (for counts 1..6, use 6)')
parser.add_argument('--max_count', type=int, default=6,
                    help='maximum count bucket; counts above this are clamped')


class CountBBoxDataset(Dataset):
    """
    Dataset for count classification from bbox CSV.
    - Image files are expected as: <ImageID>.jpg
    - Count classes are 1..max_count mapped to class indices 0..(max_count-1)
      i.e., class_idx = count - 1
    - Counts > max_count are clamped to max_count
    """

    def __init__(self, images_root, csv_path, max_count=6, transform=None):
        self.images_root = Path(images_root)
        self.csv_path = Path(csv_path)
        self.max_count = max_count
        self.transform = transform

        # 1) Count bbox rows per image
        img_to_count = defaultdict(int)
        with open(self.csv_path, "r", newline="") as f:
            reader = csv.DictReader(f)
            for row in reader:
                image_id = row["image_file_name"]
                img_to_count[image_id] += 1

        # 2) Build (img_path, class_idx) list, keeping only existing files
        self.items = []
        missing_count = 0
        bad_zero_count = 0

        for image_id, cnt in img_to_count.items():
            img_path = self.images_root / f"{image_id}.jpg"
            if not img_path.exists():
                missing_count += 1
                continue

            # You said images should have 1..6 boxes.
            if cnt <= 0:
                bad_zero_count += 1
                continue

            clamped = min(cnt, self.max_count)   # 6 means "6 or more"
            class_idx = clamped - 1             # 1..6 -> 0..5
            self.items.append((img_path, class_idx))

        if len(self.items) == 0:
            raise RuntimeError(
                f"No matched images found. Expected files like {self.images_root}/<image_file_name>.jpg"
            )

        if missing_count > 0:
            print(f"Warning: {missing_count} ImageIDs in CSV had no matching .jpg in {self.images_root}")
        if bad_zero_count > 0:
            print(f"Warning: {bad_zero_count} images had zero/invalid bbox count and were skipped")
    ...
    def __getitem__(self, idx):
        ...
        # CrossEntropyLoss expects class index as LongTensor
        target = torch.tensor(class_idx, dtype=torch.long)
        return im, target
    
FLAGS, FIRE_FLAGS = parser.parse_known_args()

ds = CountBBoxDataset(FLAGS.train_images, FLAGS.train_csv, max_count=FLAGS.max_count, transform=None)
print("Count dataset size:", len(ds))
x, y = ds[0]
print("First image size:", x.size if hasattr(x, "size") else type(x))
print("First target class idx:", int(y.item()))   # should be 0..5
...
def get_model(pretrained=False):
    ...
    in_feats = base.decoder.linear.in_features
    base.decoder.linear = nn.Linear(in_feats, FLAGS.num_count_classes)
    print("Decoder out_features:", base.decoder.linear.out_features)
...
def train(restore_path=None,  # useful when you want to restart training
    ...
    for epoch in tqdm.trange(0, FLAGS.epochs + 1, initial=start_epoch, desc='epoch'):
        ...
        for step, data in enumerate(tqdm.tqdm(trainer.data_loader, desc=trainer.name)):
            ...
            if save_val_steps is not None and global_step in save_val_steps:
                val_rec = validator()
                results[validator.name] = val_rec

                if FLAGS.output_path is not None and val_rec['loss'] < best_val_loss:
                    best_val_loss = val_rec['loss']
                    ckpt_best = {
                    ...
                    torch.save(ckpt_best, os.path.join(FLAGS.output_path, 'best_checkpoint.pth.tar'))
                    print(f"[BEST] saved best_checkpoint.pth.tar | val_loss={best_val_loss:.6f} | epoch={epoch} step={global_step}")
...
class ImageNetVal(object):
    ...
    def __call__(self):
        self.model.eval()
        start = time.time()
        record = {'loss': 0.0, 'top1': 0.0, 'top5': 0.0}
        ...
        with torch.no_grad():
            for (inp, target) in tqdm.tqdm(self.data_loader, desc=self.name):
                ...
                # loss is summed over batch because reduction='sum'
                record['loss'] += self.loss(output, target).item()
                p1, p5 = accuracy(output, target, topk=(1, 5))
                record['top1'] += p1
                record['top5'] += p5

        n = len(self.data_loader.dataset)   # number of validation images
        record['loss'] /= n
        record['top1'] /= n
        record['top5'] /= n
        record['dur'] = (time.time() - start) / len(self.data_loader)  # sec per val batch
